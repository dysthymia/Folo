...
1. **界面级真机交互未验证**：产物已重建并部署、服务在跑、页面 200，但 §6 的三个场景都还没有在真实浏览器里点过。本轮只做到「服务端行为在真实数据库副本上核对通过」+「已部署产物与构建产物逐字节一致」。浏览器验证需要 `local.folo.is` 上的登录会话（本机只有 `web-credential.json` 里的 Folo API token，不足以在无头浏览器里建立 better-auth 会话），因此留给下一轮。
2. **场景二（Blockchain 综述）连数据前提都没有**：真实业务库里 **story 总数为 0**，角色分布只有 18 条 `hidden`，没有任何 `story` / `merged` / `keeper`。也就是说「综述内联读、来源数 ≥2、句段引用」目前只有单测证据（用 fixture 造的综述），**没有一条真实综述被端到端走通**。要判定场景二，必须先让某个单分类真的产出一篇综述（需要已发布的聚合规则 + 该分类的处理轮次）。
3. **场景一的「计数恢复」未在界面核对**：`原始内容` 视图直接返回未过滤的 `rawEntryIds` 是代码级事实（`useEntriesByView`），但「数字确实跟着变」只能靠浏览器看。
4. **场景三（读前准备）未在界面核对**：「保存并启用」一次动作后「AI 处理后」直接可用、有草稿时提示「当前使用已生效版本 vN」，均为代码与单测证据。
