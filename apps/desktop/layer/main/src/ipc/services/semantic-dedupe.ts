...
interface SemanticDuplicateEntryContext {
  id: string
  title: string
  feedTitle: string
  description: string
...
interface SemanticDuplicateEvaluation {
  pairKey: string
  duplicate: boolean
  confidence: number
...
interface EvaluateSemanticDuplicatesOutput {
...
const ensureOutputSchema = async () => {
...
const runCodex = async ({
  ...
  throw lastError ?? new Error("Codex CLI is not available")
}
...
export class SemanticDedupeService extends IpcService {
  ...
  async evaluateCandidates(
    ...
    const schemaPath = await ensureOutputSchema()
